...
extern "C" fn plugin_info() -> PluginInfo {
    PluginInfo::new("adi.lang.java", "Java Language Support", env!("CARGO_PKG_VERSION"), "language")
...
extern "C" fn plugin_init(ctx: *mut PluginContext) -> i32 {
    unsafe {
        let host = (*ctx).host();
        let descriptor = ServiceDescriptor::new(SERVICE_ID, ServiceVersion::new(1, 0, 0), "adi.lang.java")
        ...
        if let Err(code) = host.register_svc(descriptor, handle) {
            host.error(&format!("Failed to register Java analyzer service: {}", code));
            return code;
        }
